class Api::V1::DashboardController < ApplicationController
    ...
    def load_recommendations
        heard_category = @recent_albums.map(&:category)
        if heard_category.present?
            @recommend_albums = Album.joins(:category, :songs).where(category: heard_category).order('songs.played_count')
